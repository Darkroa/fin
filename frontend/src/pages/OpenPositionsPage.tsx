...
const PAIRS = [
  ...
  'ADA/USDT', 'AVAX/USDT', 'LINK/USDT', 'DOT/USDT', 'UNI/USDT', 'MATIC/USDT',
  'LTC/USDT', 'XLM/USDT', 'XAU/USD', 'XAG/USD', 'OIL/WTI', 'AAPL', 'TSLA',
  'NVDA', 'MSFT', 'SPY',
];
...
export default function OpenPositionsPage() {
  ...
  const [showPairs, setShowPairs] = useState(false);        // ← Fixed name
  const [showBuySell, setShowBuySell] = useState(true);
  const [lotSize, setLotSize] = useState('0.01');
  const [leverageIdx] = useState(0);                        // ← Fixed syntax
  const leverage = LEVERAGE_STEPS[leverageIdx];
  const [orderLoading, setOrderLoading] = useState(false);
  const holdTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Missing state for realized P&L
  const [tradeHistory] = useState<any[]>([]); // TODO: Replace with real data fetching if needed
  ...
  const handleQuickTrade = async (side: 'buy' | 'sell') => {
    ...
    try {
      ...
      const d = res.data;
      toast.success(
        `${side === 'buy' ? 'Buy' : 'Sell'} ${ls} ${pair.split('/')[0]} @ market`,
        { duration: 4000 }
      );
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ...
        onMouseDown={() => {
          holdTimerRef.current = setTimeout(() => setShowBuySell((v) => !v), 3500);
        }}
        onMouseUp={() => {
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 px-3 py-2.5">
              ...
              <div className="absolute top-full mt-1 left-0 bg-[#1e2329] border border-[#2b3139] rounded-xl z-30 min-w-[140px] shadow-xl max-h-56 overflow-y-auto">
                {PAIRS.map((p) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-xs hover:bg-[#2b3139] transition ${
                      p === pair ? 'text-[#f0b90b] font-semibold' : 'text-[#eaecef]'
                    }`}
                  >
                    {p}
                  </button>
          ...
          {wsConnected && (
            <span className="w-1.5 h-1.5 rounded-full bg-[#0ecb81] animate-pulse flex-shrink-0" />
          )}

          <span
            className={`text-[10px] ml-auto ${wsConnected ? 'text-[#0ecb81]' : 'text-[#848e9c]'}`}
          >
        ...
        {showBuySell && (
          ...
          >
            ...
            <div
              className="flex items-center bg-[#0b0e11] border border-[#2b3139] rounded-lg overflow-hidden flex-shrink-0"
              onMouseDown={(e) => e.stopPropagation()}
              onTouchStart={(e) => e.stopPropagation()}
            >
              <button
              ...
              <span className="w-12 text-center text-xs font-mono text-[#eaecef] font-bold py-1.5">
                {lotSize}
              </span>
              <button
                type="button"
